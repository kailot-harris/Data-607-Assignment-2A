...
The mean rating for each movie will be calculated with missing data (NA values) removed.
...
df_2 |>
  ...
  summarize (mean_rating = mean(rating, na.rm=TRUE),
             SD_rating = stats::sd(rating, na.rm=TRUE))|>
...
Now to plot the mean and standard deviation data in a ridge plot.
...
df_2 |>
  ...
  mutate(movie_id = fct_reorder(as.factor(movie_id), 
                                rating, 
                                .fun=mean, 
                                na.rm=TRUE)) |>
  ...
  labs(title="Distribution of Ratings for Movies Directed by\nPaul Thomas Anderson (ranked by mean rating)",
       ...
       y = "Title") +
  ylab(NULL)
#Need a way to remove legend... theme?
?legend
?theme
